...
import { ISwapRouter } from "../../integrations/interfaces/uniswap/v3/ISwapRouter.sol";
import { IERC20 } from "forge-std/interfaces/IERC20.sol";
import { Execution } from "../../types/DataTypes.sol";

import { IUniswapV3Pool } from "../../integrations/interfaces/uniswap/v3/IUniswapV3Pool.sol";
import { IUniswapV3Factory } from "../../integrations/interfaces/uniswap/v3/IUniswapV3Factory.sol";
...
abstract contract InitializableUniswapV3Integration {
    ...
    function _approveAndSwap(
        ...
        exec[0] = ERC20Integration.safeApprove(tokenIn, _swapRouters[smartAccount], amountIn);
        exec[1] = _swapExactInputSingle(smartAccount, tokenIn, tokenOut, amountIn, sqrtPriceLimitX96, amountOutMinimum, fee);
    }
    ...
    function checkTokenOrder(address tokenSwappedFrom, address poolAddress) public view returns (bool swapToken0to1) {
        address poolToken0 = IUniswapV3Pool(poolAddress).token0();
        swapToken0to1 = (tokenSwappedFrom == poolToken0);
        return swapToken0to1;
    }

    function getPoolAddress(address factoryAddress, address token0, address token1, uint24 fee) public view returns (address poolAddress) {
